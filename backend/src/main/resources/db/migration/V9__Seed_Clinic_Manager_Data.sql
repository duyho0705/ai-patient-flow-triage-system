...
(uuid_generate_v4(), '00000000-0000-0000-0002-000000000001', (SELECT id FROM patient WHERE full_name_vi = 'Nguyễn Văn A' LIMIT 1), (SELECT id FROM identity_user WHERE email='doctor02@gmail.com' LIMIT 1), 'Tư vấn dinh dưỡng', 'Khám tổng quát', CURRENT_TIMESTAMP - INTERVAL '10 days', CURRENT_TIMESTAMP - INTERVAL '10 days' + INTERVAL '30 minutes'),
(uuid_generate_v4(), '00000000-0000-0000-0002-000000000001', (SELECT id FROM patient WHERE full_name_vi = 'Trần Văn Lỡ' LIMIT 1), (SELECT id FROM identity_user WHERE email='doctor02@gmail.com' LIMIT 1), 'Đau khớp', 'Thoái hóa khớp', CURRENT_TIMESTAMP - INTERVAL '60 days', CURRENT_TIMESTAMP - INTERVAL '60 days' + INTERVAL '25 minutes');
...
INSERT INTO patient (id, tenant_id, identity_user_id, full_name_vi, date_of_birth, gender, risk_level, chronic_conditions, assigned_doctor_id) VALUES 
('00000000-0000-0000-0005-000000000004', '00000000-0000-0000-0002-000000000001', '00000000-0000-0000-0005-000000000003', 'Lê Hoàng B', '1985-03-12', 'MALE', 'LOW', 'COPD', (SELECT id FROM doctor WHERE email = 'doctor02@gmail.com' LIMIT 1));

-- 4. Add data for Monthly Report historical comparison (Last Month)
-- We simulate consultations in the previous month
INSERT INTO clinical_consultation (id, tenant_id, patient_id, doctor_user_id, symptoms, diagnosis, started_at, finished_at) VALUES 
(uuid_generate_v4(), '00000000-0000-0000-0002-000000000001', (SELECT id FROM patient WHERE full_name_vi = 'Nguyễn Văn A' LIMIT 1), (SELECT id FROM identity_user WHERE email='doctor@gmail.com' LIMIT 1), 'Tái khám', 'Tiểu đường', CURRENT_TIMESTAMP - INTERVAL '35 days', CURRENT_TIMESTAMP - INTERVAL '35 days' + INTERVAL '15 minutes'),
(uuid_generate_v4(), '00000000-0000-0000-0002-000000000001', (SELECT id FROM patient WHERE full_name_vi = 'Lê Hoàng B' LIMIT 1), (SELECT id FROM identity_user WHERE email='doctor02@gmail.com' LIMIT 1), 'Khám đầu', 'Viêm phế quản', CURRENT_TIMESTAMP - INTERVAL '40 days', CURRENT_TIMESTAMP - INTERVAL '40 days' + INTERVAL '20 minutes');
