...
public interface PrescriptionRepository extends JpaRepository<Prescription, UUID> {
        ...
        @org.springframework.data.jpa.repository.EntityGraph(attributePaths = { "medications", "consultation", "consultation.patient" })
        List<Prescription> findByPatientIdOrderByCreatedAtDesc(UUID patientId);

        @org.springframework.data.jpa.repository.EntityGraph(attributePaths = { "medications", "consultation", "consultation.patient" })
        List<Prescription> findByPatientIdAndStatusOrderByCreatedAtDesc(UUID patientId, Prescription.PrescriptionStatus status);
