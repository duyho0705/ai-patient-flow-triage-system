...
public interface IdentityUserRoleRepository extends JpaRepository<IdentityUserRole, UUID> {
    ...
    @Query("SELECT COUNT(DISTINCT ur.user.id) FROM IdentityUserRole ur WHERE ur.role.code = :roleCode")
    long countDistinctUserByRoleCode(@Param("roleCode") String roleCode);
}
