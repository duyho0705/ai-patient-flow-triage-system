...
import vn.clinic.cdm.repository.patient.MedicationReminderRepository;
...
public class DoctorAiSupportController {
    ...
    private final vn.clinic.cdm.service.common.OmniChannelService omniChannelService;
    ...
    public ResponseEntity<ApiResponse<String>> sendCdmReport(
        ...
        emailService.sendEmailWithAttachment(new EmailRequest(patient.getEmail(), subject, body, pdfBytes, "Bao-cao-CDM.pdf"));
    ...
    public ResponseEntity<ApiResponse<String>> triggerMedicationReminder(@PathVariable UUID reminderId) {
        var reminder = reminderRepository.findById(reminderId)
                .orElseThrow(() -> new vn.clinic.cdm.exception.ResourceNotFoundException("MedicationReminder", reminderId));
