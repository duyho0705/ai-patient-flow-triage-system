...
import vn.clinic.cdm.dto.doctor.DoctorCreatePatientRequest;
...
@PreAuthorize("hasAnyRole('DOCTOR', 'CLINIC_MANAGER')")
public class DoctorPatientController {
    ...
    private final DoctorReportingService ReportService;
    ...
    public ResponseEntity<ApiResponse<PagedResponse<PatientDto>>> getPatientList(
        ...
        UUID userId = vn.clinic.cdm.security.AuthPrincipal.getCurrentUserId();
    ...
    public ResponseEntity<InputStreamResource> exportPatientReport(@PathVariable UUID patientId) {
        var bis = ReportService.generatePatientReportPdf(patientId);
    ...
    @PostMapping
    @Operation(summary = "Doctor tạo tài khoản và hồ sơ cho bệnh nhân")
    public ResponseEntity<ApiResponse<PatientDto>> createPatient(@Valid @RequestBody DoctorCreatePatientRequest request) {
        UUID tenantId = AuthPrincipal.getCurrentTenantId();
        UUID branchId = AuthPrincipal.getCurrentBranchId();
        
        var user = registerService.registerPatientByDoctor(
            request.getEmail(),
        ...
        var patient = patientService.getByUserId(user.getId());
        return ResponseEntity.ok(ApiResponse.success(PatientDto.fromEntity(patient)));
    }
}
